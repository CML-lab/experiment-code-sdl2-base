...
int TrackCoda::InitializeCoda(CODASYSCONFIG *CodaSysConfig)
{
	...
	if (doalign == 'y' || doalign == 'Y')
	{
		...
		if (info.dev.dwStatus != 0)
		{
			...
			std::cerr << "Alignment failed. WARNING: ALIGNMENT POTENTIALLY OUT OF DATE!" << std::endl;
	...
	cfile.getline(tmpline,sizeof(tmpline),'\n');  //get the next line of the config file
	sscanf(tmpline,"%d", &CodaSysConfig->MonitorRate);
	...
	//now that we've determined the sampling rate, we will adjust the monitor sampling rate if necessary
	if (CodaSysConfig->MonitorRate > (CodaSysConfig->cx1mode/CodaSysConfig->cx1decim))
		CodaSysConfig->MonitorRate = CodaSysConfig->cx1mode/CodaSysConfig->cx1decim;
...
int TrackCoda::GetUpdatedSample(CODASYSCONFIG *CodaSysConfig, TrackDATAFRAME DataCodaFrame[])
{
					...
					if(decode3D.canDecodePacket(packet) && decode3D.decode(packet))
					{
						...
						else
						{
							...
							for(DWORD imarker = 0; imarker < NumMarkers; imarker++)
							{
								...
								DataCodaFrame[imarker+1].CodaIntensity = DataCodaFrame[imarker+1].CodaIntensity/DataCodaFrame[imarker+1].CodaNumCameras;
								...
								//if(!DataCodaFrame[imarker+1].ValidInput)
								//	strInView[i] = '-';
