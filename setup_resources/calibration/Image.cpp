...
void Image::DrawAlign(GLfloat xPos, GLfloat yPos, GLfloat w, GLfloat h, GLfloat theta, GLint cflag)
{
	glBindTexture(GL_TEXTURE_2D, texture);
	glEnable(GL_TEXTURE_2D);
	...
	if (cflag<0)
	{
		...
		glBegin(GL_QUADS);
		glTexCoord2f(0.0f, 0.0f);
		glVertex3f(xPos - (h / 2) * sin(theta),
			yPos + (h / 2) * cos(theta), 0.0f);
		glTexCoord2f(1.0f, 0.0f);
		glVertex3f(xPos + (w) * cos(theta) - (h / 2) * sin(theta),
			yPos + (w) * sin(theta) + (h / 2) * cos(theta), 0.0f);
		glTexCoord2f(1.0f, 1.0f);
		glVertex3f(xPos + (w) * cos(theta) + (h / 2) * sin(theta),
			yPos + (w) * sin(theta) - (h / 2) * cos(theta), 0.0f);
		glTexCoord2f(0.0f, 1.0f);
