...
------------------------------------------------------------------------------
$Log: /Windows Driver/deliverable files/Bird.h $
 * 
 * 5     2/14/06 8:28a Aduling
 * 
 * 12    11/21/05 1:37p Spilc
 * Deleted default group mode - default is now group mode
 * Added optional parameter to wakeup to set group mode
 * Added optional parameter to get sys config and get device config to
 * allow getting of soft copy of configuration
 * Added birdGetErrorMessage to allow display of error messages without
 * dialog box.
 * 
 * 11    8/05/02 2:46p Kprei
 * Blars -   It was determined that some of the Ascension commands, such
 * as
 * the Change/Examine Value command, will frequently fail for post-3.67
 * EPROMs unless the transmitter is set to pulsed mode.  Consequently, the
 * routines birdRS232WakeUp() and birdISAWakeUp() once again automatically
 * set the transmitter to pulsed mode before returning.  This un-does the
 * driver change made on 2/21/01, and restores the one made on 7/15/99.
 * Also, it was determined that the overhead associated with setting up a
 * system timer, introduced on 1/18/01 for use by the
 * birdStartFrameReadyCallback() routine,is prohibitive for some
 * applications, leading to measurement cycle overruns.  As a result, the
 * driver has been modified so that the systemtimer is installed only when
 * the calling application invokes birdStartFrameReadyCallback().  If this
 * routine is never called, then no system timer will be installed.
 * 
 * 11    1/28/02 10:29a Kprei
 * Blars -   Added safeguards against calling routines out of sequence.
 * 
 * 10    2/26/01 3:43p Kprei
...
#define MOTIONSTAR_TCP_PORT	6000	// motionstar tcp port
#define MOTIONSTAR_UDP_PORT	5000	// motionstar udp port
...
/*
	...
	Remarks:       Wakes up a group of birds in RS232 mode.  
                  "nGroupID" can be any number between 0 and BIRD_MAX_GROUP_ID 
						that the user wants to associate with this group of 
						birds.  
                  "bStandAlone" indicates if the group consists 
						...
						relevant when bStandAlone = FALSE.)  
                  "pwComport" points to an array of words, each of which is the
						...
						comport number of zero.  
                  "dwBaudRate" is the baud rate to use.  
                  "dwReadTimeout" is the maximum time, in msecs, that 
                  the application will take when trying to receive a 
                  character.  
                  "dwWriteTimeout" is the maximum time, in msecs, that the
                  application will take when trying to transmit a character.
                  "nGroupMode" defines whether or not the data sampling will
                  happen in group mode.  In group mode all the data is passed 
                  back via the Master com port (1).  In non-group mode all the 
                  data is passed back via dedicated com ports.  Non-group mode
                  is the only time that the pwComport array values with indices of
                  greater than 1 are used.
...
BOOL DLLEXPORT birdGetDeviceConfig(int nGroupID, int nDeviceNum, 
								   BIRDDEVICECONFIG *pdevcfg, BOOL bGetDriverCopy = FALSE);
...
/*
	...
	Remarks:            Sets up a callback function to be called whenever
						...
						WARNING: Use of this routine causes the driver to
						install a system timer, which increases the total amount
						of processing overhead.  Depending on the processor
						speed, measurement rate, and number of birds in the flock, 
						this may lead to measurement cycle overrun errors.
*/
...
/*
	birdGetErrorMessage Get Error message text

	Parameters Passed:  none

	Return Value:       LPSTR to message text

	Remarks:            

*/
LPSTR DLLEXPORT birdGetErrorMessage();
